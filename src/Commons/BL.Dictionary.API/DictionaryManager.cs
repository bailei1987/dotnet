...
namespace BL.Dictionary
{
    public class DictionaryManager
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(dto.K) || string.IsNullOrWhiteSpace(dto.V) || string.IsNullOrWhiteSpace(dto.Type)) throw new Exception("k,v,type cant be null");
            if (Coll.CountDocuments(x => x.Type == dto.Type && (x.K == dto.K || x.V == dto.V)) > 0) throw new Exception("this item already exist");
            Coll.InsertOne(dto);
        }
        public void Put(DictionaryItem dto)
        {
            if (string.IsNullOrWhiteSpace(dto.Id)) throw new Exception("id cant be null");
            var obj = Coll.Find(x => x.Id == dto.Id).SingleOrDefault();
            if (obj is null) throw new Exception("id is not correct");
            if (obj.Type != dto.Type) throw new Exception("type cant be modified");
            if (string.IsNullOrWhiteSpace(dto.K) || string.IsNullOrWhiteSpace(dto.V) || string.IsNullOrWhiteSpace(dto.Type)) throw new Exception("k,v,type cant be null");
            if (Coll.CountDocuments(x => x.Id != dto.Id && x.Type == dto.Type && (x.K == dto.K || x.V == dto.V)) > 0) throw new Exception("this item already exist");
            obj.K = dto.K;
            obj.V = dto.V;
            obj.Order = dto.Order;
            obj.Desc = dto.Desc;
            Coll.ReplaceOne(x => x.Id == dto.Id, obj);
        }

        public DictionaryItem Get(string id)
        {
            return Coll.Find(x => x.Id == id).SingleOrDefault() ?? throw new Exception("id is not correct");
        }
